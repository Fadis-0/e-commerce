...
const Signup = () => {
	const { role, setRole, handlePopup, popupMessage, setPopupMessage, popupType, setPopupType } = useStateContext();
	...
	function handleSubmit(event) {
		...
		const formData = new FormData(form);
		  if (profile === 'supplier') {
			fetch('http://192.168.0.138:8080/suppliers/login', {
			  method: 'POST',
			  body: formData,
			  credentials: 'include',
			})
			  ...
			  .then((data) => {
					...
					const user = data.user;
                     user.role = 'supplier'; // Add the role property to the user object
			...
			fetch('http://192.168.0.138:8080/userRole/login', {
			  method: 'POST',
			  body: formData,
			  credentials: 'include',
			})
				...
				<div className="mt-20 flex-col">
					<div className="px-10 h-10 relative flex items-center mb-8">
						...
						<div className="w-full ml-8 h-full flex justify-between items-center">
							<div className={`shadow-sm hover:shadow group hover:border-lime-600 relative w-12 h-12 flex justify-center cursor-pointer items-center rounded-full border-2 border-color ${profile === "admin" ? "bg-lime-100 border-lime-600" : "bg-white"}`} onClick={() => handleSetProfile("admin")}>
							...
							<div className={`relative group hover:border-indigo-600 w-12 h-12 rounded-full flex justify-center items-center cursor-pointer border-2 border-color ${profile === "store" ? "bg-indigo-100 border-indigo-600" : "bg-white"}`} onClick={() => handleSetProfile("store")}>
